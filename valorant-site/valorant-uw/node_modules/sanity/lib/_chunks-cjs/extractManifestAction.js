"use strict";
var node_crypto = require("node:crypto"), fs = require("node:fs/promises"), path = require("node:path"), node_worker_threads = require("node:worker_threads"), chalk = require("chalk"), dateFns = require("date-fns"), readPkgUp = require("read-pkg-up"), timing = require("./timing.js"), _internal = require("./_internal.js");
...
async function extractManifestSafe(args, context) {
  if (EXTRACT_MANIFEST_ENABLED)
    try {
      ...
      return _internal.SCHEMA_STORE_FEATURE_ENABLED || context.output.print(chalk__default.default.gray(`\u21B3 Couldn't extract manifest file. Sanity Create will not be available for the studio.
  Disable this message with ${FEATURE_ENABLED_ENV_NAME}=false`)), EXTRACT_MANIFEST_LOG_ERRORS && context.output.error(err), err;
    }
}
