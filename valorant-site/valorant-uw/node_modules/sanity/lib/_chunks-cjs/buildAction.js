...
async function buildSanityApp(args, context, overrides) {
  const timer = timing.getTimer(), {
    ...
    flags,
    cliConfig
  }), installedSdkVersion = await readModuleVersion.readModuleVersion(context.workDir, "@sanity/sdk-react"), installedSanityVersion = await readModuleVersion.readModuleVersion(context.workDir, "sanity");
...
Largest module files:`), output.print(shouldAutoUpdate.formatModuleSizes(shouldAutoUpdate.sortModulesBySize(bundle.chunks).slice(0, 15))));
  } catch (err) {
